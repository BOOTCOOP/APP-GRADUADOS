        ...
        >
          ...
          <template #default="{ items }">
            <ion-list class="ion-margin-top">
              <Job :job="job" v-for="job in filteredJobs(items)" :key="job.id"></Job>
            </ion-list>
          </template>
        </InfinitePagination>
      </ion-content>
...
<script setup lang="ts">
  import { IonList, IonIcon, IonText, IonButton, IonContent, useIonRouter } from '@ionic/vue';
  ...
  const showFilters = () => {
    ...
    store.dispatch("ui/action/show", [
      {
        text: currentModality === 'Presencial' ? '✓ Modalidad: Presencial' : 'Modalidad: Presencial',
        handler: () => {
          modalityFilter.value = currentModality === 'Presencial' ? '' : 'Presencial';
        }
      },
      {
        text: currentModality === 'Híbrido' ? '✓ Modalidad: Híbrido' : 'Modalidad: Híbrido',
        handler: () => {
          modalityFilter.value = currentModality === 'Híbrido' ? '' : 'Híbrido';
        }
      },
      {
        text: currentModality === 'Home office' ? '✓ Modalidad: Home office' : 'Modalidad: Home office',
        handler: () => {
          modalityFilter.value = currentModality === 'Home office' ? '' : 'Home office';
        }
      },
      {
