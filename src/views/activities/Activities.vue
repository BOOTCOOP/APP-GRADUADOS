    ...
    <graduados-app header-title="Talleres">
        ...
        <div class="search-filter-container ion-margin-bottom">
            ...
            >
                ...
                Filtros
                <ion-badge color="primary" v-if="activeFiltersCount > 0" class="filter-badge">
                    {{ activeFiltersCount }}
                </ion-badge>
            </ion-button>
        </div>
...
import "swiper/css";
import { IonButton, IonIcon, IonText, IonList, IonBadge, useIonRouter, actionSheetController } from '@ionic/vue';
...
async function openCategoryFilter() {
    const actionSheet = await actionSheetController.create({
        ...
        buttons: [
            { text: 'Todas las temáticas', handler: () => { selectedCategory.value = 'all'; } },
            { text: 'Legal', handler: () => { selectedCategory.value = 'legal'; } },
            { text: 'Académico', handler: () => { selectedCategory.value = 'academico'; } },
            { text: 'Profesional', handler: () => { selectedCategory.value = 'profesional'; } },
            { text: 'Tecnología', handler: () => { selectedCategory.value = 'tecnologia'; } },
            { text: 'Gestión', handler: () => { selectedCategory.value = 'gestion'; } },
...
async function openModalityFilter() {
    const actionSheet = await actionSheetController.create({
        ...
        buttons: [
            { text: 'Todas las modalidades', handler: () => { selectedModality.value = 'all'; } },
            { text: 'Presencial', handler: () => { selectedModality.value = 'presencial'; } },
            { text: 'Virtual', handler: () => { selectedModality.value = 'virtual'; } },
            { text: 'Híbrida', handler: () => { selectedModality.value = 'hibrida'; } },
            { text: 'Cancelar', role: 'cancel' }
        ]
    });
    await actionSheet.present();
...
async function openMonthFilter() {
    const actionSheet = await actionSheetController.create({
        ...
        buttons: [
            { text: 'Todos los meses', handler: () => { selectedMonth.value = 'all'; } },
            { text: 'Enero', handler: () => { selectedMonth.value = '01'; } },
            { text: 'Febrero', handler: () => { selectedMonth.value = '02'; } },
            { text: 'Marzo', handler: () => { selectedMonth.value = '03'; } },
            { text: 'Abril', handler: () => { selectedMonth.value = '04'; } },
            { text: 'Mayo', handler: () => { selectedMonth.value = '05'; } },
            { text: 'Junio', handler: () => { selectedMonth.value = '06'; } },
            { text: 'Julio', handler: () => { selectedMonth.value = '07'; } },
            { text: 'Agosto', handler: () => { selectedMonth.value = '08'; } },
            { text: 'Septiembre', handler: () => { selectedMonth.value = '09'; } },
            { text: 'Octubre', handler: () => { selectedMonth.value = '10'; } },
            { text: 'Noviembre', handler: () => { selectedMonth.value = '11'; } },
            { text: 'Diciembre', handler: () => { selectedMonth.value = '12'; } },
            { text: 'Cancelar', role: 'cancel' }
        ]
    });
    await actionSheet.present();
...
function categorizeActivity(activity: any): string {
    ...
    // Legal
    const legalKeywords = ['derecho', 'legal', 'jurídico', 'jurisprudencia', 'abogado', 'tribunal', 'corte', 'legislación', 'normativa', 'código', 'ley', 'decreto', 'constitucional', 'procesal', 'civil', 'penal', 'laboral', 'comercial'];
    if (legalKeywords.some(keyword => fullText.includes(keyword))) {
        return 'legal';
    }
    
    // Académico
    const academicKeywords = ['investigación', 'académico', 'tesis', 'maestría', 'doctorado', 'universidad', 'facultad', 'cátedra', 'seminario', 'metodología', 'científico', 'estudio', 'análisis', 'teoría'];
    if (academicKeywords.some(keyword => fullText.includes(keyword))) {
        return 'academico';
    }
    
    // Tecnología
    const techKeywords = ['tecnología', 'digital', 'informática', 'software', 'programación', 'datos', 'sistema', 'plataforma', 'web', 'aplicación', 'internet', 'ciberseguridad', 'ia', 'inteligencia artificial'];
    if (techKeywords.some(keyword => fullText.includes(keyword))) {
        return 'tecnologia';
    }
    
    // Gestión
    const managementKeywords = ['gestión', 'administración', 'management', 'liderazgo', 'recursos', 'proyectos', 'estrategia', 'planificación', 'organización', 'dirección'];
...
function extractModality(activity: any): string {
    ...
    if (fullText.includes('virtual') || fullText.includes('online') || fullText.includes('distancia')) {
        return 'virtual';
    }
    if (fullText.includes('presencial') || fullText.includes('aula')) {
        return 'presencial';
    }
    if (fullText.includes('híbrida') || fullText.includes('mixta') || fullText.includes('combinada')) {
        return 'hibrida';
    }
