    ...
    <graduados-blank body="white" :hideFabButton="true">        
        <Form ref="form" class="content">
            ...
            <div>
                ...
                <Field name="password" v-model="password" label="contraseña" v-slot="{ field }" rules="required">
                ...
                <div>
                    ...
                    <ion-text color="medium" router-link="/recuperar-contrasena" class="ion-margin-vertical"><small>¿Olvidaste tu contraseña?</small></ion-text>
                </div>
            </div>
            <div>
                <ion-button @click="login()" :disabled="sending" expand="full" shape="round">{{ sending ? 'Ingresando...' : 'Ingresar' }}</ion-button>                
                <div class="ion-text-center">
                    <ion-text color="medium">¿Aún no tenés una cuenta? <span @click="register()">Registrate</span> </ion-text>
                </div>
            </div>
        </Form>
    </graduados-blank>
...
<script setup lang="ts">
    ...
    function login(){
        form.value.validate().then( v => {
            if(v.valid){
                sending.value = true;
                useAuth().login(email.value, password.value).then(() => {
