            ...
            <div class="options ion-margin-top">
              <div class="info">
                <div>
                  ...
                  <ion-text color="medium"> {{ course.teachers }}</ion-text>
                </div>
                <div class="ion-margin-start">
                  <ion-icon :md="calendarOutline" :ios="calendarOutline" color="medium"></ion-icon>
                  <ion-text color="medium"> {{ course.start }}</ion-text>
                </div>
              </div>
              <ion-icon v-if="course.is_only_for_graduado_uba" style="font-size:20px" :md="starOutline" :ios="starOutline" color="primary"></ion-icon>
            </div>
          </ion-card-content>
    </ion-card>
</template>
...
function showDetail(){
    router.push({name:'courses.show', params: {slug: prop.course.id}})
}
</script>
