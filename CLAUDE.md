...
Todo lo que sale de la app (playlist de YouTube = "Actividades Online", WhatsApp de consultas, redes, y la base pública `PUBLIC_WEB_URL` + `publicUrl()` que usan los links para compartir) vive en [src/uses/externalLinks.ts](src/uses/externalLinks.ts) y se abre con `window.open(url, '_system')` para que el SO lo resuelva con la app nativa. Dos reglas que ya costaron bugs: **WhatsApp usa `wa.me`** (no `web.whatsapp.com`, que fuerza el cliente web en el celular) y **"Actividades Online" abre la playlist directo** tanto desde el acceso rápido del inicio como desde el menú — la ruta `/classifieds` quedó solo para deep-links viejos de notificaciones y ya no exige sesión.
