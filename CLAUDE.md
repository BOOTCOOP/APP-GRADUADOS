...
OTA self-hosted con `@capgo/capacitor-updater` en modo manual: al montar la app llama `notifyAppReady()` (rollback automático si un bundle crashea antes) y después chequea el manifiesto `public/ota/latest.json` servido por Pages; los zips van en GitHub Releases (tag `bundle-x.y.z`). Force update: la app compara su versión contra `min_version` de `GET /api/app/config` (y ante un 426 de la API) → alert bloqueante hacia la tienda. **Regla de oro**: `npm run build:native` (base `/`) para Capacitor/OTA, `npm run build` (base `/APP-GRADUADOS/`) solo para Pages — nunca sincronizar un build web a Capacitor. Detalle completo en [docs/releases-y-actualizaciones.md](docs/releases-y-actualizaciones.md).
